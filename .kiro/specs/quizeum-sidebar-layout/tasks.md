...
## 9. Phase 28: PC版サイドバー表示切り替えおよびミニ表示時のツールチップ表示（2026-06-22）

- [x] 9.1 (P) PC表示時におけるサイドバー表示切り替え機能と余白調整の実装
  - レイアウトラッパー内でサイドバーの折りたたみ状態（初期値: 非折りたたみ）を管理し、サイドバーに状態と切り替え関数を Props で渡す。
  - 折りたたみ状態が有効の際、PC表示（1024px以上）におけるメインコンテンツの左側パディングを 275px から 70px へ動的に縮小する。
  - 折りたたみ状態が有効 of 際、PC表示におけるサイドバーの幅を 275px から 70px へ動的に縮小する。
  - サイドバーが折りたたまれている際、ロゴテキストの "eum" やナビゲーション項目のテキストラベル、フッターのユーザー名テキストを非表示にする。
  - **完了状態**: PCサイズ表示において折りたたみが有効の際、サイドバーが 70px 幅に縮小し、メインコンテンツが重ならずに余白も 70px へ自動で連動し、再度切り替えると元に戻ること。リロードや初期表示時はデフォルトで通常表示（非折りたたみ）に戻ること。
  - _Requirements: 9.2, 9.3_
  - _Boundary: LayoutWrapper, Sidebar_

- [ ] 9.2 (P) サイドバー折りたたみトグルボタンと直接マイページ遷移の実装
...
- [ ] 9.4 Phase 28 ナビゲーションおよびレイアウトのテスト更新
...
- [ ]* 9.4* Phase 28 E2E 拡張（任意・MVP後）
  - 折りたたみ切り替え時のアニメーション動作の滑らかさや、キーボード操作（タブフォーカス時のツールチップ表示など）のアクセシビリティに関するテストを記述する。
  - **完了状態**: キーボード操作によるフォーカス時のツールチップ動作を含むアクセシビリティ検証テストがパスすること。
  - _Requirements: 9.4_
  - _Depends: 9.4_
  - _Boundary: Testing_
...
- **実装順**: 9.1、9.2、9.3 は並行可能（同一ファイルの編集になるためマージ競合に配慮する）。9.4 は 9.1〜9.3 のすべてが完了した後に実行。9.4* は任意。
- **ツールチップの構築**: Radix UI などの外部ライブラリは使用せず、Tailwind CSS の `group relative` と `absolute` 位置調整による CSS ベースのツールチップとすることで軽量に実装する。

...
* **UI実装方針（作問・プレイ共通）**:
  ...
  - **プレイ画面**（`/quiz/[id]/play`）: 問題切り替え時に `sortingItems` を Fisher–Yates 等でシャッフルし、ソート可能リストにバインド。各カード左にドラッグハンドル（`aria-grabbed` 等）を配置し、ドラッグ中は `transform` / `transition` による視覚フィードバックを付与する。
* **UIアクションとステート制御**:
  - 問題に紐づく `sortingItems` 配列（2〜6要素）を取得し、表示順を初期シャッフルした状態でドラッグ＆ドロップ可能なリストにバインドします。
...
* **プレイ画面**（`ChoiceAnswerPanel` / `/quiz/[id]/play` 等）:
  - 問題の `choices` のうち `isCorrect === true` の件数が **1 件** のときは **ラジオボタン**、**2 件以上** のときは **チェックボックス** で選択肢を表示する。
...
* **正誤判定ロジック (Client-side / API)**:
  ...
  - 正規化された入力値が、問題オブジェクトに登録されている正解パターンリスト（`correctTextAnswerList`）のいずれかの要素と完全一致（または部分一致）しているかを検証します。
...
* **クイズ詳細画面 (`/quiz/[id]`)**:
  - `quiz.format === 'quick-press'` または問題に `type === 'quick-press'` が含まれる場合、模擬試験・フラッシュカードの選択 UI を非表示とし、通常モード固定の案内カードを表示します。
  - 「解答後にその場で正誤・解説を表示」チェックボックス（デフォルト ON）の状態をクエリ `feedback=true|false` としてプレイ URL に付与します（`/quiz/[id]/play?mode=normal&feedback=...`）。
* **カンニング防止（クライアント難読化）**:
  - プレイ画面・弱点克服（復習）画面のクイズ読み込み時、`type === 'quick-press'` の問題について `questionText` および `correctTextAnswerList` の各要素を `btoa(unescape(encodeURIComponent(...)))` で Base64 エンコードし、DOM 上の平文露出と DevTools による先読みを抑止します。表示・判定時は `atob` + `decodeURIComponent(escape(...))` で復号します。
* **UI フロー（1 問題あたり）**:
...
* **タイムの永続化と結果画面**:
  ...
  - 結果画面 (`/quiz/[id]/result`) で読み込み後 `removeItem` し、平均・最速・正答数の統計カードおよび問題別バッジを表示します。
* **ユーザー回答の記録と結果画面表示**:
  - `usePlayState` の `handleAnswerSubmit` は、正誤判定後に `setQuestionAnswers(prev => ({ ...prev, [questionId]: rawAnswer }))` で問題ごとのユーザー回答を State に蓄積する。`rawAnswer` は、選択式なら選択肢ID（複数正解時はカンマ区切りID列）、並び替えならカンマ区切りID列、記述式/早押しなら入力テキスト、フラッシュカードなら `'correct'`/`'incorrect'`。
...
sequenceDiagram
    ...
    Note over QC: format 選択<br>mixed / 単一形式(選択式・記述式・早押し・並び替え・連想・水平思考)<br>単一形式時は全問題 type を自動固定<br>形式変更時：問題が全初期値なら確認ダイアログなし即切替<br>入力済みの問題があれば確認ダイアログ表示
    
    alt 下書き保存の場合 (isPublished = false)
        Creator->>QC: 「下書き保存」をクリック
        QC->>QC: 下書きバリデーション (タイトル・ジャンル・各問題 questionText)
        ...
        QC->>QC: 問題数(>=1)、各問題正解設定、format と問題 type の一貫性、文字数制限（水平思考のAI用コンテキストは20〜2000文字）などを検証
        Note over QC: validateQuizForPublish<br>・genre 必須<br>・各問題 questionText 必須（5〜500文字）<br>・mixed: multiple-choice / true-false / text-input / sorting のみ<br>・単一形式: 全問題 type が format と一致<br>・text-input: correctTextAnswerList 必須<br>・text-input + char-count: textInputCharCount 1〜100、正解候補文字数一致<br>・text-input + numeric: 正解候補はすべて数値<br>・multiple-choice: choices 2〜10件 かつ正解1件以上（複数正解可）<br>・true-false: choices 2件固定<br>・エラーは該当フィールド直下にも表示（questionText 含む）
        ...
        alt バリデーション失敗
            ...
            alt NGワード検知 (true)
                ...
                QS->>DB: トランザクション開始
                DB->>DB: 各問題を questions コレクションに独立保存
...
* **呼び出し元**:
  - **公開**: `validateQuizForPublish` → `collectQuestionValidationErrors` の先頭で各問題に適用。
...
* **テストプレイとの関係**: F-206 は「トリム後1文字以上の問題文がある問題が1問以上」で開始可能（5文字ルールは**適用しない**）。下書き保存・公開とは要件を分離する。テストプレイ画面でも問題文は本番プレイと同様にマークダウンレンダリングする。
...
sequenceDiagram
    ...
    Creator->>QE: 問題・メタ情報を編集中
    Creator->>QE: 「テストプレイ」をクリック
    QE->>QE: 最小バリデーション（問題文入力済み問題 >= 1）
    alt プレイ可能な問題が0件
        QE-->>Creator: 「テストプレイするには、問題文が入力された問題を1問以上追加してください」
    else プレイ可能
...
* **ペイロード構造**:
  ...
  ```
* **エディタ → ペイロード変換**: `quiz-editor.tsx` の保存処理と同じ正規化（問題 ID 付与、format 反映）を適用するが、`validateQuizForPublish` および NG ワードチェックは**スキップ**する。
...
* 良問評価・体感難易度投票・間違い指摘・作家リアクション・SNS 共有・問題ブックマーク・マイリスト追加 UI は**非表示**。
...
| 問題 `correctCount` / `incorrectCount` 更新 | **実行しない**            |
...
sequenceDiagram
    ...
    Creator->>QE: 問題を編集・修正し「公開更新」をクリック
...
プレイ画面のレイアウトは **2カラム構成** とし、ウミガメのスープ問題プレイ時の専用表示を行います。
...
### 1.6 問題単体のブックマークおよびマイリスト追加フロー
ユーザーがクイズプレイ中または結果画面等から、特定の問題（Question）を個別でお気に入り登録（ブックマーク）したり、自身の作成したリストに追加・削除するフローです。アトミックなトランザクションによりデータ整合性を担保します。

```mermaid
sequenceDiagram
    autonumber
    actor Player as プレイヤー
    participant QP as 問題表示コンポーネント (Play/Result UI)
    participant QService as QuestionService (src/services/question.ts)
    participant DB as Firestore (Database)

    alt 問題単体のブックマーク登録（トグルON）
        Player->>QP: 「問題をブックマーク」をクリック
...
**Phase 8 ガード**: 問題ブックマーク・リスト追加はいずれも親クイズ `status === 'published'` を必須とする。問題リスト以外への `addQuestionToList` は `ListTypeMismatchError`。

---

### 1.7 リスト種別・問題リストプレイ・参照リンク作問（Phase 8）
...
- クイズリスト操作（`quizIds` CRUD）と問題リスト操作（`questionIds` CRUD）は `assertListTypeOperation` で分離。

#### 1.7.2 問題リスト連続プレイ
1. リスト詳細で `getQuestionsInList` により順序付きメンバーを取得。
2. `question-list-session`（sessionStorage）に `listId` と進行インデックスを保持。
3. 各問題プレイ完了時に `saveAttempt({ mode: 'question-list', listId, quizId: 親ID, totalQuestions: 1, ... })`。
4. 結果画面から次問題へ遷移。最終問題完了後にセッションをクリア。

#### 1.7.3 参照リンク作問（Copy-on-Write）
1. `searchAuthorQuizzes` で自作クイズを検索し、問題を `linkKind: 'reference'` でエディタに追加。
...
クイズのカバー画像、問題画像、ユーザーアバター、ジャンル新設時のアイコンなど、様々なアセットがアップロードされる Firebase Storage の統合セキュリティ設計方針です。
...
* **アセットパス命名規則**:
  ...
  - 問題参考画像: `/quizzes/{quizId}/questions/{questionId}_{timestamp}.png`
  - ユーザーアバター: `/users/{uid}/avatar_{timestamp}.png`
  - ジャンルアイコン（新設申請）: `genre-icons/{uid}_{timestamp}.png`（拡張子は実ファイルに合わせる。PNG/JPEG/GIF のみ）
* **容量・ファイル制限バリデーション**:
...
#### 6.4.1 タグ別クイズ一覧取得 (`getQuizzesByTag` 処理ロジック / Phase 6)

* **処理フロー** (`src/services/quiz.ts`):
  1. タグ文字列を正規化し、`resolveCanonicalTagIds` で正規タグ ID（例: `react`）を決定する。
  2. **第1クエリ（高速）**: `where('status','==','published')` ＋ `where('canonicalTagIds','array-contains', canonicalTagId)` ＋ `orderBy`（新着/人気/トレンド）で取得。
  3. **第2クエリ（レガシー）**: 旧フィールド `tags` に対する `array-contains` で未移行クイズを補完し、Map で ID 重複除去後にソート・件数切り詰め。
* **設計意図**: 書き込み時に埋め込んだ `canonicalTagIds` を優先し、検索時の `array-contains-any` 展開を不要にする。

#### 6.4.2 ジャンル別クイズ一覧取得 (`getQuizzesByGenre` 処理ロジック / Phase 6 C2)

* **処理フロー** (`src/lib/metadata-resolution.ts` + `src/services/quiz.ts`):
  1. `resolveCanonicalGenreId(genreName)` で正規ジャンル ID を決定（`canonicalId` チェーンを辿る）。
  2. **第1クエリ（高速）**: `where('canonicalGenreId','==', canonicalId)` ＋ `status` ＋ `orderBy` で取得。
  3. **第2クエリ（レガシー）**: `expandGenreIdsForQuery` が返す ID 集合（正規 ID + 入力 ID + `mergedGenreIds`）を `genre in`（10件チャンク）で取得しマージ。
  4. 重複除去・`sortQuizzesForList` 適用後、上限件数で返却。
* **ホーム複合検索**: `searchQuizzes` は `filters.genreId` 指定時に上記 `getQuizzesByGenre` をベース（最大100件）とし、難易度・問題数・キーワードはアプリ層でフィルタ。プレイ状況は `GET /api/user/played-quiz-ids` + `applyPlayStatusFilter` で後段適用。

#### 6.4.3 探索 UI・作問 UI のジャンルマスタ連携（Phase 6）

| 画面 / コンポーネント    | データ源                                        | 挙動                                                                                                          |
| :----------------------- | :---------------------------------------------- | :------------------------------------------------------------------------------------------------------------ |
| ホーム `GenreNav`        | `listActiveGenres` / `useActiveGenres`          | アイコンタップは **一覧フィルタではなく** `/genres/[genreId]` へ遷移                                          |
| ホーム複合検索           | `GenreSearchField` + `filter-genre-suggestions` | `displayName` / ID の部分一致サジェスト。フィルタ変更時 `searchQuizzes`（約300ms デバウンス）                 |
| `/genres/[genreName]` 等 | `getQuizzesByGenre` + `ExploreSortTabs`         | 新着・人気・トレンド切替                                                                                      |
| 作問エディタ             | `GenreEditorSelect` + `useActiveGenres`         | ハードコード選択肢を廃止。フォーカス時 `refetch` でマスタ最新化                                               |
| ジャンル新設申請         | `genre-icon-upload.ts`                          | `validateGenreIconFile` / `GENRE_ICON_ACCEPT`（PNG/JPEG/GIF、2MB、SVG 禁止）を Storage アップロード前後で共有 |
...
- **権限**: 認証済みユーザー全員
- **仕様**: ユーザーは、「こんなジャンルのクイズを投稿したいが、適当なジャンルがない」場合に、希望するジャンルID（英語物理キー）、日本語表示名、および説明を入力し、さらに**そのジャンルを象徴する「正方形の画像アイコン（PNG/JPEG/GIF形式、SVGは不可）」を Firebase Storage にアップロード**します（SVG-based XSS防御のためSVG形式は完全排除）。
  - **クライアント検証**: `src/lib/genre-icon-upload.ts`（`validateGenreIconFile`, `GENRE_ICON_ACCEPT`, `assertGenreIconFileValid`）
  - **Storageパス**: `genre-icons/{uid}_{timestamp}.png`（JPEG/GIF の場合は拡張子を実 MIME に合わせる）
- **データ作成**:
...
- **プレイ画面**（`play/page.tsx`）、**弱点克服**（`/quiz/review`）、**フラッシュカード**、**模擬試験**、**テストプレイ**（`/quiz/test-play/play`）: 問題カード上部の問題文表示に `parseMarkdownToHtml(currentQuestion.questionText)` を適用する。
- **早押し（`quick-press`）**:
...
- **水平思考（`lateral-thinking`）**: チャット上部の問題サマリー表示にも同一パイプラインを適用する。プレイヤーが AI へ送る自由記述（最大100文字）は問題文とは別フィールドであり、マークダウン対象外。

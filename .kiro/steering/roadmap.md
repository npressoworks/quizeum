...
### Boundary Strategy（本フェーズ）
- **quizeum-core**: `Quiz.visibility` 型、デフォルト、マイグレーション、`canViewQuiz`、`createQuiz`/`updateQuiz` Pro ゲート、全フィード/検索/タイムラインクエリ、Rules/Indexes、attempt/play API ゲート
- **quizeum-ui-editor** / **quizeum-creator-dash-ui**: エディタ metadata に公開範囲 UI（Radio/Select）、無料時の disabled + 警告、Pro 誘導
- **quizeum-ui-discovery** / **quizeum-play-flow-ui**: フィードは変更不要（Core クエリ側で除外）。詳細・プレイの未認可 UX
- **quizeum-ui-quiz-lifecycle**: 詳細・プレイ・OGP layout の `canViewQuiz` 統合
- **quizeum-billing-subscription-ui**: Pro 特典文言、ダウングレード時の説明（任意: 設定/料金 FAQ）
- **quizeum-auth-profile-ui**: プロフィールのクイズ一覧バッジ、他人プロフィールでは `public` のみ（既存 `getQuizzesByAuthor(authorId, false)` 拡張）
- **quizeum-my-quiz-ui**: 自作ソースは下書き・非公開含む（現状維持、visibility バッジ追加）
...
- [ ] quizeum-core -- `visibility` 型・デフォルト、`canViewQuiz`、`assertCanSetQuizVisibility`、全公開フィード/検索/TL クエリ、create/update Pro ゲート、Rules/Indexes、マイグレーション、play/attempt API ゲート。Dependencies: none
- [ ] quizeum-ui-editor -- エディタ公開範囲セレクタ、無料時警告・Pro 誘導、保存時エラーハンドリング。Dependencies: quizeum-core
- [ ] quizeum-ui-quiz-lifecycle -- 詳細・プレイ・OGP のアクセス拒否 UX、`canViewQuiz` 統合。Dependencies: quizeum-core
- [ ] quizeum-ui-discovery -- （軽微）作者プロフィール・カードで visibility バッジが必要なら対応。Dependencies: quizeum-core
- [ ] quizeum-play-flow-ui -- 未認可プレイ開始の 403 UX、ブックマーク連携確認。Dependencies: quizeum-core
- [ ] quizeum-billing-subscription-ui -- Pro 特典に「非公開クイズ」追記、料金画面 copy 更新。Dependencies: quizeum-core
- [ ] quizeum-auth-profile-ui -- プロフィール一覧の visibility フィルタ（他人は public のみ）、バッジ。Dependencies: quizeum-core
- [ ] quizeum-creator-dash-ui -- ダッシュボード一覧の visibility バッジ・フィルタ（任意）。Dependencies: quizeum-core
...
- **In**:
  ...
  - **プレイ中のトラッキング**:
    ...
    - `quizeum-play-flow-ui` での解答詳細トラッキングと attempt 保存 payload への組み込み
  - **保存・検証・同期**:
...
### Boundary Strategy（本フェーズ）
- **quizeum-core**: `Attempt` 型の拡張、`saveAttempt`・オフライン同期（`attempt-session`）でのデータモデルバリデーションと検証、インデックス整備、Firestore Security Rules の確認
- **quizeum-ui-quiz-lifecycle** / **quizeum-play-flow-ui**: クイズプレイ画面（通常・試験・フラッシュカード等）における問題ごとのタイマー計測、ヒント・選択肢変更トラッキング、解答詳細構造の組み立てと送信
- **quizeum-analytics-bigquery** (新規): BigQuery スキーマ定義、過去データ一括インポートスクリプト、Firebase Extension 設定マニュアル
...
- [ ] quizeum-core -- `Attempt` スキーマ拡張、`saveAttempt` での解答詳細バリデーション、オフライン同期スキーマ拡張。Dependencies: none
- [ ] quizeum-ui-quiz-lifecycle -- プレイ中 UI でのタイマー・ヒント・変更トラッキング、解答詳細の組み立てと `saveAttempt` 呼び出し。Dependencies: quizeum-core
...
### Specs (dependency order)
- [ ] quizeum-analytics-bigquery -- BigQuery スキーマ定義、インポートスクリプト、Extension設定。Dependencies: quizeum-core

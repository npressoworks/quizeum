...
4. The Quiz Editor shall ジャンル選択に `data-testid="genre-editor-select"` を維持する。
...
### Requirement 4: リストエディタ UI
**Objective:** As a クリエイター, I want クイズリストまたは問題リストを作成・編集できること, so that キュレーションコンテンツを公開できる。

#### Acceptance Criteria
1. The List Editor shall タイトル、説明、カバー画像 URL、公開トグル、リストタイプ（クイズリスト/問題リスト）の入力 UI を提供する。
2. The List Editor shall リストタイプ選択に `data-testid="list-type-selector"`, `list-type-quiz`, `list-type-question` を維持する。
3. When クイズリストタイプが選択されているとき, the List Editor shall クイズ検索・添付・HTML5 DnD による並べ替え UI を提供する。
4. When 問題リストタイプが選択されているとき, the List Editor shall 問題添付パネルを表示する。
5. When 新規問題リストを保存し listId が未確定であるとき, the List Editor shall 保存後に編集ルートへリダイレクトする既存フローを維持する。
6. The List Editor shall リスト保存・JSON エクスポートのアクションボタンを提供する。
7. While エディタがデータ読込中であるとき, the List Editor shall `data-testid="list-editor-skeleton"` のスケルトン表示を行う。

### Requirement 5: 問題リスト添付パネル
**Objective:** As a クリエイター, I want 公開済み・ブックマーク・公開探索から問題を検索してリストに添付できること, so that 問題リストを効率的に構築できる。

#### Acceptance Criteria
1. The Question Attach Panel shall `data-testid="question-list-attach-panel"` をルート要素に維持する。
2. When listId が未確定であるとき, the Question Attach Panel shall 添付操作を無効化し `question-attach-disabled-hint` を表示する。
3. The Question Attach Panel shall 3 タブ（own-published, bookmarked, public-explore）による問題検索 UI を提供し、各タブに `question-attach-tab-{name}` を維持する。
4. The Question Attach Panel shall キーワード検索に `question-attach-keyword` を維持する。
5. When ユーザーが問題を添付または解除したとき, the Question Attach Panel shall `attach-question-{id}` / `attached-question-{id}` の `data-testid` を維持する。
6. When 添付済み問題が存在するとき, the Question Attach Panel shall HTML5 DnD による並べ替え UI を提供する。
...
6. When リスト保存時にタイトルが未入力であるとき, the List Editor shall クライアント側バリデーションエラーを表示する。
7. When リスト保存が成功したとき, the List Editor shall 既存の成功通知と遷移（新規問題リスト時は編集ルート）フローを維持する。
8. The Quiz Editor and List Editor shall 保存/公開/下書き/エクスポートボタンの `data-analytics` 属性を既存配置から削除しない。
...
#### Acceptance Criteria
1. When 本スペックの実装が完了したとき, the Editor UI shall `create.module.css`, `edit.module.css`, `sortable-sorting-list.module.css`, `markdown.module.css`, `editor-skeleton.module.css`, `list-skeleton.module.css` を削除する。
2. The Editor UI shall 既存ルート（`/quiz/create`, `/quiz/[id]/edit`, `/list/create`, `/list/[id]/edit`）と loader コンポーネントのデータ取得契約を変更しない。
3. When 本スペックの実装がマージされたとき, the Editor UI shall `e2e/quiz-creation.spec.ts` と `e2e/quiz-list.spec.ts` がグリーンである。
